...
public class RolapStar {
    ...
    /**
     * Retrieves the value of the cell identified by a cell request, if it can
     * be found in the local cache of the current statement (thread).
     *
     * <p>
     * If it is not in the local cache, returns null. The client's next step
     * will presumably be to request a segment that contains the cell from the
     * global cache, external cache, or by issuing a SQL statement.
     *
     * <p>
     ...
     *
     * <p>
     * If <code>pinSet</code> is not null, pins the segment that holds it into
     * the local cache. <code>pinSet</code> ensures that a segment is only
     * pinned once.
     *
    ...
    private Object getCellFromExternalCache(CellRequest request) {
        final SegmentWithData segment
                = Locus.peek().getServer().getAggregationManager().cacheMgr.peek(request);
        if (segment == null) {
            return null;
        }
    ...
    /**
     * Temporary. Contains the local cache for a particular thread. Because it
     * is accessed via a thread-local, the data structures can be accessed
    ...
    /**
     * Decrements the column counter; used if a newly created column is found to
     * already exist.
     */
    ...
    /**
     * Place holder in case in the future we wish to be able to reload
     * aggregates. In that case, if aggregates had already been loaded, i.e.,
     * this star has some aggstars, then those aggstars are cleared.
     */
     ...
     * <code>joinColumn</code> is specified, only considers child tables joined
     * by the given column.
     */
     ...
     * @param columnNameList List of column names (must have same cardinality as
     * {@code columnList})
    ...
    public static class Column {
        ...
        /**
         * When a Column is a column, and not a Measure, the parent column is
         * the coloumn associated with next highest Level.
         */
         ...
         * dimension or unshared dimension with the same column names, this is
         * used to disambiguate aggregate column names.
         */
        ...
        /**
         * The estimated cardinality of the column. {@link Integer#MIN_VALUE}
         * means unknown.
         */
        ...
        /**
         * Get column cardinality from the schema cache if possible; otherwise
         * issue a select count(distinct) query to retrieve the cardinality and
     ...
     * and a condition which specifies how it is joined to its parent. So the
     * star schema is, in effect, a hierarchy with the fact table at its root.
     */
    public static class Table {
         ...
         *
         * <p>
         * Note: This method is slow, but that's acceptable because it is only
         * used for tracing. It would be more efficient to store an array in the
        ...
        /**
         * Extends this 'leg' of the star by adding <code>relation</code> joined
         * by <code>joinCondition</code>. If the same expression is already
         * present, does not create it again. Stores the unaliased table names
         * to RolapStar.Table mapping associated with the input
         * <code>cube</code>.
         */
         ...
         * @param failIfExists Pass in false if you might have already added the
         * table before and if that happens you want to do nothing.
    ...
    public static class Condition {
        ...
        public String getLeft(final SqlQuery query) {
            ...
            return this.getAsFunctionSupportedQueryString(this.left, query);
        }
        ...
        public String getRight(final SqlQuery query) {
            ...
            return this.getAsFunctionSupportedQueryString(this.right, query);
        }

        public String toString(SqlQuery query) {
            //
            // Feature change added by DOCOBO to support
            // joins in functions
            //
            // return left.getExpression(query) + " = "
            //    + right.getExpression(query);
            return getAsFunctionSupportedQueryString(left, query) + " = "
                    + getAsFunctionSupportedQueryString(right, query);
        }
         ...
         * @return
         */
        private String getAsFunctionSupportedQueryString(MondrianDef.Expression leftOrRight, final SqlQuery query) {

            String retVal = "";
            //
            // DOCOBO Fix : by default 
            //  getExpression(query) is same as 
            // query.getDialect().quoteIdentifier(table, name);
            //
            //
            // If the left name contains a simple bracket
            // and the name of the table in "`" quotes 
            // it is assumed to be a function
            //
            boolean checkIfFunctionJoin = false;
            //
            // Putting in a try catch to make sure 
            // on failure it does the default
            //
            try {
                if(leftOrRight instanceof MondrianDef.Column){
                    MondrianDef.Column leftOrRightMondrianDefCol = (MondrianDef.Column)leftOrRight;
                    
                    checkIfFunctionJoin = leftOrRightMondrianDefCol.getColumnName().contains("(")
                        && leftOrRightMondrianDefCol.getColumnName().contains("`" + leftOrRight.getTableAlias() + "`.");
                    if(checkIfFunctionJoin){
                        // Use whatever is defined in the name
                        retVal = leftOrRightMondrianDefCol.getColumnName();
                    }
                }
            } catch (Exception exc) {
                // DO nothing on expection
            }
            if (!checkIfFunctionJoin) {
                retVal = leftOrRight.getExpression(query);
            }
    ...
    /**
     * Comparator to compare columns based on their name and table that contains
     * them
     */
